...
function make_kernel() {
	...
	if [ "$KDIR" == "$KERNEL_OUT" ]; then
		time make -C $KDIR $cc  -j12 ${MAKE_ARGS}
		time make -C $KDIR $cc -j12 INSTALL_MOD_PATH=$MODULES_OUT modules_install
	else
		time make -C $KDIR O="$KERNEL_OUT" $cc -j12 ${MAKE_ARGS}
		time make -C $KDIR O="$KERNEL_OUT" $cc -j12 INSTALL_MOD_PATH=$MODULES_OUT modules_install
	fi
